...
import { useLoadPdf, useFile, useCurrentPage, useStoreHydration } from './services/pdf.service'
...
export const PdfOcr: FC = () => {
  ...
  const { loadPdf, data: loadPdfData, status: loadPdfStatus, error: loadPdfError } = useLoadPdf()
  ...
  if (ocrLoadStatus === 'error' || storeError)
    return (
      <ErrorMessage
        message={ocrLoadingError?.message || storeError?.message || 'Error loading OCR engin'}
      />
    )
